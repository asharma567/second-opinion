...
unset GEMINI_API_KEY  # 429 quota — drop fatal provider before discovery (sibling override)
fi
...
pick_role() {
  ...
  local preferred=""
  case "$role" in
    author_a)    preferred="codex grok openrouter" ;;
    critic)      preferred="grok openrouter codex" ;;
    author_b)    preferred="openrouter grok codex" ;;
    synthesizer) preferred="grok openrouter codex" ;;
  esac
  local p q
